...
all_player = [['','','',''] for i in range(10)] # 10 個房間的玩家，每房最多 4 人
play_ready = [[-1,-1,-1,-1] for i in range(10)] # 已經準備開始
...
def register_player(sid, data): # 玩家進入房間
    ...
    for i in range(len(all_player)):
        if uid in all_player[i]:
            try:
                ...
                # 若房內無玩家(0-3)了，則解鎖且其餘人移除
                if all(p == '' for p in all_player[i][0:4]):
                    lock_room[i] = False
                    gameover_count[i] = 0
            except ValueError:
                pass
    ...
    # 3. 進入目標房間 (前四名為玩家，其餘為觀戰)
    p_idx = -1
    for i in range(4):
        if all_player[room_num][i] == '':
            all_player[room_num][i] = uid
            p_idx = i
            break
...
def leave_room(sid,data): # 玩家退出房間
    ...
    try:
        player_list = all_player[room_num]
        if uid in player_list:
            player_index = player_list.index(uid)
            if player_index < 4:
    ...
    # 檢查是否房內無玩家了
    if all(p == '' for p in all_player[room_num][0:4]):
...
def room_ready(sid, data): # 房間準備開始
    ...
    if p_idx < 4:
        ...
        # 檢查是否可以開始 (至少兩人，且所有現有玩家都 ready)
        players_in_slots = [i for i, name in enumerate(all_player[room_num][0:4]) if name != '']
        if len(players_in_slots) >= 2 and all(play_ready[room_num][i] != -1 for i in players_in_slots):
...
@sio.event
def request_gameover_sync(sid, data):
    room_num = data['room_num']
    # Once requested, force everyone in the room to gameover state
    play_ready[room_num] = [-1,-1,-1,-1]
    lock_room[room_num] = False
    gameover_count[room_num] = 0
    sio.emit('gameover_sync', {}, room=room_num)
    sio.emit('lock_room', {'lock_room_list': lock_room})
...
def gameover(sid,data):
    ...
    players_count = len([name for name in all_player[room_num][0:4] if name != ''])
    if gameover_count[room_num] >= players_count: # 所有玩家都掛掉
        play_ready[room_num] = [-1,-1,-1,-1]
        lock_room[room_num] = False
        gameover_count[room_num] = 0
        sio.emit('gameover_sync', {}, room=room_num) # Final insurance
        sio.emit('lock_room',{'lock_room_list': lock_room})
